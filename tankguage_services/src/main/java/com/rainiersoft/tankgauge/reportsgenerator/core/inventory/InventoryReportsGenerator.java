...
public class InventoryReportsGenerator extends TankGaugingReportsGenerator {

	public InventoryReportsGenerator(Statement sqlStatement) throws Exception {
		...
		InventoryPDFReportsGenerator genericPDFRepGen = null;
		...
		inventorySQLQuery = inventorySQLQuery + " WHERE DATE_FORMAT(TKD.last_updated, '%Y-%m-%d %k:%i') = " + "\"" +
				TankGaugingReportsGenerator.reportStartDateAndTime + "\" ORDER BY TKD.TANK_ID ASC";
		...
		sqlRRProcessor = new InventoryReportsProcessor();
		...
		genericPDFRepGen = new InventoryPDFReportsGenerator();
		
		// Generic header manager
		genericHeaderGenMgr = new InventoryHeaderManager();
		...
		// Populate entire file path for external systems to access the file
		TankGaugingReportsGenerator.fullFilePath = 
				repConfigProperties.getProperty(InventoryReportConstants.INVENTORY_PDF_REPORT_PATH_NAME) + autoFileName;
